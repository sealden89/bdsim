...
BDSElectronQuantumLevel::BDSElectronQuantumLevel(G4int    nPrincipleNumberIn,
                                                 G4int    lAngularNumberIn,
                                                 G4double jSpinOrbitCouplingIn):
  ...
  timeOfExcitement(0)
{
  maxOccupancy = ((G4int)jSpinOrbitCoupling*2)+1;
  levelEnergy = DiracEnergyNJ(nPrincipleNumberIn, (G4int)jSpinOrbitCouplingIn);
}

G4double BDSElectronQuantumLevel::DiracEnergyNJ(G4int n, G4int j) const
{
